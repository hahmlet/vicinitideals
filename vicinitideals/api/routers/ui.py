...
from vicinitideals.models.deal import Deal, DealModel, DealOpportunity, DealStatus, IncomeStream, IncomeStreamType, OperatingExpenseLine, OperationalInputs, ProjectType, UnitMix, UseLine, UseLinePhase
from vicinitideals.models.ingestion import DedupCandidate, DedupStatus, IngestJob, RecordType, SavedSearchCriteria
from vicinitideals.models.org import User
from vicinitideals.models.capital import CapitalModule, DrawSource, WaterfallTier
from vicinitideals.models.cashflow import OperationalOutputs
...
from vicinitideals.models.milestone import DEFAULT_DURATIONS, Milestone, MilestoneType, MilestoneType as MT
from vicinitideals.models.project import Opportunity, OpportunitySource, OpportunityStatus, Project, ProjectBuildingAssignment, ProjectParcelAssignment, ProjectStatus
from vicinitideals.models.property import Building, BuildingStatus, OpportunityBuilding, Property
from vicinitideals.models.scraped_listing import ScrapedListing
from vicinitideals.models.realie_usage import RealieUsage
from vicinitideals.scrapers.realie import _current_month
...
async def admin_rlis_refresh(
    ...
    """
    from vicinitideals.tasks.parcel_seed import rlis_quarterly_refresh_task
    user = await _get_user(session, vd_user_id)
    _require_settings_owner(user)
...
async def admin_seed_rlis(
    ...
    from vicinitideals.tasks.parcel_seed import seed_rlis_task
    user = await _get_user(session, vd_user_id)
    _require_settings_owner(user)
...
async def admin_backfill_listing_buckets(
    ...
    from vicinitideals.utils.priority import classify as _classify
...
async def admin_classify_parcels(
    ...
    from vicinitideals.tasks.parcel_seed import classify_parcels_task
    user = await _get_user(session, vd_user_id)
    _require_settings_owner(user)
    ...
    if opportunity is None:
        ...
        for _sl in opportunity.scraped_listings:
            ...
            if _parcel_id is None and (_sl.apn or _sl.address_normalized):
                try:
                    from vicinitideals.scrapers.parcel_enrichment import enrich_parcel as _enrich
...
async def opportunity_wizard_step(
    ...
    if step == 1:
        ...
        if opp is None:
            from vicinitideals.models.org import Organization as _Org
...
async def opportunity_detail(
    ...
    user = await _get_user(session, vd_user_id)
    dedup_count = await _get_dedup_count(session)
    from vicinitideals.models.parcel import Parcel as _Parcel
...
async def dissolve_opportunity(
    ...
    from vicinitideals.models.parcel import ParcelTransformation
    from vicinitideals.models.portfolio import GanttEntry
    from vicinitideals.models.org import ProjectVisibility
    from vicinitideals.models.project import PermitStub
...
async def parcel_gis_refresh(request: Request, parcel_id: UUID, session: DBSession) -> HTMLResponse:
    ...
    from vicinitideals.scrapers.parcel_enrichment import enrich_parcel
...
async def map_context(
    ...
    """
    from vicinitideals.utils.gis import (
...
async def promote_listing(
    ...
    from vicinitideals.tasks.scraper import _promote_listing as _do_promote, _get_default_org_id  # local import avoids circular
...
async def promote_listing_redirect(
    ...
    from vicinitideals.tasks.scraper import _promote_listing as _do_promote, _get_default_org_id
...
async def _load_builder_data(session: AsyncSession, model_id: UUID, project_id: UUID | None = None) -> dict:
    ...
    from vicinitideals.models.cashflow import CashFlow as _CashFlow, PeriodType as _PT
    ...
    from vicinitideals.models.milestone import MilestoneType as MT
    ...
    if default_project:
        ...
        if hold_period is not None:
            try:
                ...
                from vicinitideals.models.milestone import Milestone as _Milestone
...
async def deal_setup_wizard_complete(
    ...
    from vicinitideals.models.capital import CapitalModule, FunderType
...
async def model_builder(
    ...
    if active_module == "cashflow":
        from vicinitideals.models.cashflow import CashFlow
...
async def download_model_export(
    ...
    from vicinitideals.exporters.excel_export import export_deal_model_workbook, make_export_filename
    model = await session.get(DealModel, model_id)
    if model is None:
        return HTMLResponse("Not found", status_code=404)
...
async def model_builder_line_form(
    ...
    if type in ("capital_modules", "sources", "use_lines", "uses"):
        from vicinitideals.models.project import Project as _LFProject
        from vicinitideals.models.milestone import Milestone as _LFMilestone
...
async def _load_draw_schedule_ctx(
    ...
    from vicinitideals.models.milestone import Milestone
...
async def calculate_draw_schedule(
    ...
    from vicinitideals.engines.draw_schedule import UseLineItem
