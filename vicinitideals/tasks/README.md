...
| `scraping` | Listing ingestion through LXC 134 Scrapling | `vicinitideals.tasks.scraper.scrape_listings` |
| `analysis` | Post-ingestion modeling and analytics | `vicinitideals.tasks.scenario.run_scenario`, `vicinitideals.tasks.scenario.sweep_variable` |

Any task under `vicinitideals.tasks.scraper` is routed to the `scraping` queue.
Any task under `vicinitideals.tasks.scenario` is routed to the `analysis` queue.
...
```bash
celery -A vicinitideals.tasks.celery_app call vicinitideals.tasks.scraper.scrape_listings --args '["loopnet", {}]'
```

### Scenario analysis

```bash
celery -A vicinitideals.tasks.celery_app call vicinitideals.tasks.scenario.run_scenario --args '["<scenario-uuid>"]'
celery -A vicinitideals.tasks.celery_app call vicinitideals.tasks.scenario.sweep_variable --args '["<scenario-uuid>"]'
```
...
- Config source: `vicinitideals.config.Settings.scrape_interval_hours`
- Beat sends both sources to `vicinitideals.tasks.scraper.scrape_listings`
