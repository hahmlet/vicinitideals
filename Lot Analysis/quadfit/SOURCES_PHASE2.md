...
| Milwaukie | 845 | **A — wired 2026-09-02, and the only chapter in the corpus that yields TWO actions.** `Willamette_Greenway`/8 (n=2) is a KILL: MMC 19.401.3 makes all development in the WG zone a conditional use under 19.905 and 19.401.5.B's thirteen exemptions do not reach a dwelling. `Habitat_Conservation_Areas`/7 (n=1), `Vegetated_Corridors`/6 (n=61) and `Wetlands`/5 (n=26) are FLAGS: 19.402.5.A prohibits development "other than those allowed by Section 19.402", and Table 19.402.3.K allows limited WQR disturbance for new dwelling units at Type I. `NR_100ft_Compliance`/10 is the 19.402.3.A applicability band and is deliberately NOT screened — inside it, outside the resource, all the code asks for is a construction management plan | **A** `Floodplain`/9 (n=173), `COM_FEMA_Hazards`, `Flood_1996` — not wired, FEMA NFHL covers flood everywhere | — regional DEM |
| Clackamas uninc. | 598 | **C — wired 2026-09-02 as FLAGS off the REGIONAL layers.** The county publishes no HCA or WQRA geometry of its own (its AGOL org carries only `GeoHazard`, `Zoning`, `Rural_/Urban_Land_Use_Zoning`), and ZDO 706.02 says the HCA Map comes from Metro's 2002 inventory anyway. ZDO 706.05 and 709.05 each prohibit exactly two things and a dwelling is neither, so both are flags | NFHL 41005C; ZDO 703 Floodplain Management District not read — FEMA already flags/carves everywhere | regional DEM |
| West Linn | 355 | **A — wired 2026-09-02.** `RiparianCI`/2 is the adopted 2014 map and it is CENTRELINES: 378 STREAM (carve 65 ft, Table 32-2 row A), 17 EPHEMERAL (carve 15 ft, row F), 190 PIPED (flag — no row states a width). `RiparianCI`/1 RCI polygons flagged; `WetlandInventory`/1 (n=91) carved at 65 ft | **A** `FloodManagement`/1 flood management area — FLAG, CDC 27.040 prohibits only base-zone uses and hazmat | **A** `SteepSlope2014`/0 — not wired; the DEM screen already grades slope |
| Wilsonville | 140 | **B** `LandUseDataset/SROZ_ImpactArea`/166 (n=22) — published as the 25 ft ring with the SROZ core as the polygon hole; `fill_holes` recovers it. `Map___NaturalResources` appears in the service list but errors — the earlier row was wrong | **A** `cow_flood_100`, `cow_flood_500`, `FLOOD_FEMA`, `greenway` | regional DEM |
...
| Tualatin | 0 (19 review) | **A/B — wired 2026-09-03, and the best environmental publication in the corpus.** The path in the row above was wrong: `Public/` answers *Token Required* and `EnvironmentalExplorer` sits at the service ROOT. `/23` is the adopted NRPO district itself (n=159, six CLASS values) — CARVE, TDC 72.060(1) prohibits "no building, structure, grading, excavation, placement of fill, vegetation removal, impervious surface, use, activity or other development" and 72.060(2) lets back only paths, streets, utilities, parks and landscaping. `/26` is the 50 ft stream buffer, which 72.040(3)(a) writes into the DEFINITION of an Open Space Natural Area — CARVE on the 85 unpiped segments, FLAG on the 8 piped (72.040(3)(b) measures from bankfull stage; a pipe has none). `/25` is the Wetlands Protection District split by the sub-area names Chapter 71 defines — CARVE on 17 (protected area, Sweek Pond, and the three classes of 71.061's 40 ft setback), FLAG on the 2 Wetlands Fringe Area polygons, which 71.062(2) expects to be developed "in all areas". `/3` is the city's own 25% slope layer, deliberately NOT wired — the DEM already measures slope over the buildable envelope and one fact deserves one measurement | NFHL 41005C; TDC 70 Floodplain District not read — FEMA already flags/carves everywhere | **A** `/3` `/4` published and not used, see left |
| Gladstone | 0 (121 review) | **C — wired 2026-09-03 as FLAGS off the REGIONAL layers, like the county.** "No greens at stake" was the wrong test: Gladstone's lots are held by nothing but signing, so 51 of the 121 would have gone green on unscreened resource land. The city publishes no HCA geometry — its HCA Map is adopted inside Comprehensive Plan Ch. 6 and its zoning service (hosted in Oregon City's `GLADSTONE/` folder) carries base zones only, 35 polygons, no overlay districts. `gladstone_hca` reads Metro Title 13, `gladstone_wq` Metro Title 3. Both flags: GMC 17.25.050 prohibits exactly invasive planting and outside storage, ZDO 706.05 word for word. **17.28 GW is a KILL that cannot be drawn** — every development in the greenway is a conditional use (17.28.030) with a setback set "on a case-by-case basis" (17.28.050(6)), and the only publisher of the Willamette River Greenway boundary is OPRD, as a POLYLINE | NFHL 41005C; GMC 17.29's own extension to the February 1996 flood footprint (17.29.030(3)) NOT separated — it exists as FLOOD96 on the Metro Title 3 layer already on disk | regional DEM |
...
- Tualatin: `https://tualgis.ci.tualatin.or.us/server/rest/services/` — the ROOT. The `Public/` folder
  recorded here until 2026-09-03 answers `Token Required`, and `EnvironmentalExplorer` is not in it.
  A folder that refuses to list is not a service that refuses to answer.
