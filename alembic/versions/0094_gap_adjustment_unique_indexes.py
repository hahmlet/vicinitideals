...
def upgrade() -> None:
    # cash_flow_line_items.income_stream_id FKs the loser phantom rows; null
    # them out before the dedupe DELETE so the FK does not block the
    # cleanup. These are computed output rows regenerated by the next
    # cash flow compute. operating_expense_lines and use_lines have no
    # inbound FKs to worry about.
    op.execute(
        sa.text(
            """
            UPDATE cash_flow_line_items
            SET income_stream_id = NULL
            WHERE income_stream_id IN (
                SELECT a.id
                FROM income_streams a
                JOIN income_streams b
            ...
            """
        ).bindparams(label=_REVENUE_LABEL)
    )
