"""Write reading-queue decisions from the review inbox into the rule files.

Rules load from the repository. A decision made in a browser lands in
``flats.reading_rulings`` and has not taken effect until it is here, in a
...
Run it::
    ...
    uv run python scripts/flats_drain_reading_rulings.py            # report
    uv run python scripts/flats_drain_reading_rulings.py --write    # and write
    uv run python scripts/flats_drain_reading_rulings.py --layer or/multnomah/gresham
...
from app.models.flats import FlatsReadingRuling  # noqa: E402
from flats.encode.worklist import card_key, rule  # noqa: E402


async def pending(
    session: AsyncSession, layer: str | None = None
) -> list[FlatsReadingRuling]:
    ...
    """
    stmt = select(FlatsReadingRuling).where(FlatsReadingRuling.exported_at.is_(None))
    if layer:
        stmt = stmt.where(FlatsReadingRuling.layer == layer)
    rows = list(
        (await session.execute(stmt.order_by(FlatsReadingRuling.decided_at)))
        .scalars()
        .all()
    )
    latest: dict[tuple[str, str, str], FlatsReadingRuling] = {}
    for row in rows:
        latest[(row.layer, row.path, row.section)] = row
...
async def drain(layer: str | None, write: bool) -> int:
    async with AsyncSessionLocal() as session:
        ...
        superseded = 0
        for row in rows:
            key = card_key(row.path, row.section)
            print(f"{row.layer}  {key}  [{row.queue}/{row.outcome}]  by {row.decided_by}")
        ...
        if write:
            ...
            stale = select(FlatsReadingRuling).where(
                FlatsReadingRuling.exported_at.is_(None)
            )
            if layer:
                stale = stale.where(FlatsReadingRuling.layer == layer)
