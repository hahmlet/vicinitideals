"""Write fetch-triage decisions from the review inbox into the rule files.

Rules load from the repository. A decision made in a browser lands in
``flats.crossref_rulings`` and has not taken effect until it is here, in a
...
Run it::
    ...
    uv run python scripts/flats_drain_crossref_rulings.py            # report
    uv run python scripts/flats_drain_crossref_rulings.py --write    # and write
    uv run python scripts/flats_drain_crossref_rulings.py --layer or/multnomah/gresham
...
from app.models.flats import FlatsCrossrefRuling  # noqa: E402
from flats.encode.triage import rule  # noqa: E402


async def pending(
    session: AsyncSession, layer: str | None = None
) -> list[FlatsCrossrefRuling]:
    """Undrained decisions, oldest first, latest-per-reference only.
    ...
    """
    stmt = select(FlatsCrossrefRuling).where(
        FlatsCrossrefRuling.exported_at.is_(None)
    )
    if layer:
        stmt = stmt.where(FlatsCrossrefRuling.layer == layer)
    rows = list(
        (await session.execute(stmt.order_by(FlatsCrossrefRuling.decided_at)))
        .scalars()
        .all()
    )
    latest: dict[tuple[str, str], FlatsCrossrefRuling] = {}
    for row in rows:
        latest[(row.layer, row.ref)] = row
...
async def drain(layer: str | None, write: bool) -> int:
    async with AsyncSessionLocal() as session:
        ...
        superseded = 0
        for row in rows:
            print(f"{row.layer}  {row.ref}  [{row.outcome}]  by {row.decided_by}")
            print(f"    {row.note[:150]}")
            if not write:
                continue
            try:
                path = rule(row.layer, row.ref, row.outcome, row.note)
        ...
        if write:
            ...
            stale = select(FlatsCrossrefRuling).where(
                FlatsCrossrefRuling.exported_at.is_(None)
            )
            if layer:
                stale = stale.where(FlatsCrossrefRuling.layer == layer)
