...
def test_the_industrial_chapter_does_not_swallow_its_own_table(
    ...
    # The chapter's other unmarked body was a screening note a thousand lines
    ...
    # landscaping standard and not a marker at all. Losing that false pointer
    # is what made the report true, and it left the note with nothing real
    # pointing at it either, because the marker on the table did not survive
    # extraction.
    #
    # It survives now. `BRACKET_MARKER` reads a run -- "[24,25]" is two marks
    # and not one number -- so the marker is seen where it is printed and the
    # chapter reconciles.
    assert [b.line for b in got.unmarked] == []
