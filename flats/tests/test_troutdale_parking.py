...
def test_the_refused_ledger_carries_every_standard_this_model_cannot_hold(
    ...
    """Eight refusals against ten encoded values, and the ratio is the reading.
    ...
    #
    # NINE -> EIGHT on 2026-09-10, and this is the good direction. The ninth
    # was MU-3's twenty-five foot MINIMUM building height (TDC 8.230.A), added
    # by the blind second reading on 2026-09-07 as a shape this model could not
    # store -- `max_height_ft` is a ceiling and there was no counterpart. There
    # is one now: `min_building_height_ft` holds it, MU-3 quotes it, and the
    # refusal is a value. A refusal should only ever leave this ledger by being
    # encoded, which is why the number is dropped here rather than the
    # assertion loosened.
    assert len(mine) == 8
