...
def test_a_skipped_key_has_to_already_be_here() -> None:
    ...
    for jname, spec in load_quadfit()["jurisdictions"].items():
        ...
        for row in spec.get("zones") or []:
            for key in (BACKPORTED - {"lot_size_bands"} - set(HELD_AS_VARIANT)) & set(row):
        ...
        for row in spec.get("zones") or []:
            ...
            for field, bands in (row.get("lot_size_bands") or {}).items():
                ...
                for threshold, value in bands:
                    ...
                    checked += 1
    assert checked == 27, f"expected 27 banded rows, walked {checked}"
...
def test_written_config_loads_through_the_real_loader() -> None:
    ...
    # single unencoded zone in either county among them.
    #
