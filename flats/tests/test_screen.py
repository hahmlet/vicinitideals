...
Three asymmetries define this module and every test here defends one of them:

* an unverified rule set can never produce RED — a bad number in a YAML file
  must not delete an acquisition target;
* tolerance rescues a lot into REVIEW and never certifies one into GREEN;
...
def run(rule_set=None, lot=LOT, f=None):
    return screen(rule_set or rules(), lot, DESIGN, f or fit(), policy=POLICY)
...
def test_a_clean_lot_under_verified_rules_is_green() -> None:
    ...
    assert result.fit_slack_ft == pytest.approx(4.0)


def test_every_check_reports_its_margin_even_when_it_passes() -> None:
    # Ranking needs the margins, not just the verdict.
    names = {c.check for c in run().checks}

    assert "fit_ft" in names
    assert "coverage_pct" in names
...
def test_unverified_rules_route_to_review() -> None:
    result = run(rules(RuleVerdict.unverified))

    assert result.triage is Triage.review
...
def test_unverified_rules_cannot_produce_a_red() -> None:
    ...
    result = run(rules(RuleVerdict.unverified, min_lot_sqft=99_000))

    assert result.triage is Triage.review
    assert any(c.verdict is Verdict.fails for c in result.checks), "the miss is still recorded"


def test_an_unencoded_zone_routes_to_review_with_its_reason() -> None:
...
def test_an_unencoded_jurisdiction_routes_to_review() -> None:
...
def test_a_prohibition_nobody_verified_is_still_only_a_review() -> None:
...
def test_a_lot_below_the_minimum_area_is_red() -> None:
...
def test_a_pod_too_big_for_the_envelope_is_red() -> None:
...
def test_a_miss_inside_measurement_noise_is_review_not_red() -> None:
    # One raster cell of shortfall. It buys a review, never a deletion.
...
def test_a_lot_with_no_street_found_is_reviewed_not_dropped() -> None:
    result = run(
...
def test_an_unreadable_shape_is_reviewed() -> None:
    result = run(
...
def test_parking_compares_what_the_design_provides_to_what_the_code_demands() -> None:
    ...
    short = run(rules(parking_min_per_unit=2.0))
