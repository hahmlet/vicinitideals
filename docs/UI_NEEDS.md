...
| ~~B~~ | ~~Anchor-driven dates (2d1)~~ — **✅ shipped 2026-04-23**: `resolve_project_start_dates` walks each anchor chain, pivots on the parent's milestone end date, applies `offset_months` + `offset_days`. Engine seeds the resolved date as a delta-shift across the project's `milestone_dates`. Timeline Anchors panel on the Underwriting tab provides the editor. Cycle detection rejects bad edges at write time. Byte-identical on zero-anchor scenarios. | |
...
- **[2026-04-23, phase-2d1]** Anchor-driven project start dates. `resolve_project_start_dates()` walks each anchor chain (`project_anchors.anchor_milestone_id` + `offset_months` + `offset_days`), engine shifts the anchored project's `milestone_dates` by the computed delta. New Timeline Anchors panel on the Underwriting tab provides an `<optgroup>`-grouped milestone dropdown + offset inputs. Cycle check rejects P1→P2→P1 at write time with a readable error. Byte-identical on zero-anchor scenarios (every prod deal today).
...
- **[2026-04-22, phase-3a]** Calc-status pill HTMX endpoint returning 422 — the `@router.get` decorator had been attached to the sync helper `_render_calc_status_pill_html` instead of the async handler `model_calc_status_pill` below it.
