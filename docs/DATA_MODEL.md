...
| Entity | Table | Purpose | Key |
|---|---|---|---|
| **FlatsSnapshot** | `flats.snapshots` | One dated copy of the county map (migration 0132): the acquire manifest verbatim, per-dataset counts, the RLIS release, who promoted it. Exactly one row is `current`; a refresh lands as `candidate` beside it (HUMAN_TODO 20). A candidate load ends by writing `checks` — the promotion gate from `flats/ingest/checks.py`, `{code: {tripped, detail}}` for `layers_incomplete`, `count_drift`, `new_zones`, `lots_drift`, `zone_changes`, `rlis_agreement` — and adds `lots` / `measured` / `by_reason` / `new_zones` / `baseline_snapshot_id` to `counts`, which the *next* refresh compares against | `id` (bigint); unique `(snapshot_date, host)`; partial unique on `status = 'current'` |
...
| **FlatsRun** | `flats.runs` | One pipeline execution; records code + rule versions, designs and counties in scope, and the snapshot it read. `status` is `running` / `complete` / `failed` / `candidate` (loaded from a refreshed copy, reachable by `?run=`, never the default) / `retired` (its copy's lot rows pruned). Promotion (`app/services/flats_refresh.py`, migration 0134) turns the candidate `complete`; the Lots pages default to the newest complete run | `id` (bigint); `snapshot_id` |
| **FlatsDesign** | `flats.designs` | Immutable snapshot of a catalog pod as a run used it | `key` = `id@version` |
| **FlatsLot** | `flats.lots` | A taxlot **as one snapshot holds it** and every **design-independent** fact about it (envelope, fit frontier, slope, sewer, economics — JSONB `facts`). Each snapshot owns its rows, so a refresh never overwrites the copy in use. A snapshot-fed run carries **every** lot of the map: a measured lot's `facts` are quadfit's plus `assessor` (the roll: land / building / total / assessed value, year built, building sqft, last sale, property code, state class, land use), `condo` and `snapshot_zone`; an unmeasured lot's `facts` are `{source: "snapshot", unmeasured: {reason, quadfit_step}, …}` and its results are `unknown` with that reason. `condo_verdict` is the normalize stage's (`land` / `suspect`) | `(snapshot_id, county, tlid)` unique; `(county, tlid)` indexed |
...
| **FlatsReviewDecision** | `flats.review_decisions` | Durable human verdict, replayed into every later run. `needs_rereview_snapshot_id` + `needs_rereview_reason` (migration 0134) are set by promotion when the promoted copy shows the decision's lot split, merged, renumbered, deleted, vacated or rezoned: the decision stands, marked "look again" | partial unique on `(county, tlid, design_key, check_code)` where not superseded |
...
| **FlatsWordRuling** | `flats.word_rulings` | Inbox for a word-review decision about what one *word* means in one jurisdiction — the queue under signing, since a number read perfectly out of its sentence is wrong if the city measures that sentence's words its own way (standing + outcome + required note); drained into the jurisdiction YAML as a `words:` block | `id` (bigint), append-only, latest `decided_at` per `(layer, term)` wins; `term` is our vocabulary, not the city's spelling; `fingerprint` reopens the card when the glossary is re-extracted |
| **FlatsTaxSnapshot** | `flats.tax_snapshots` | One property-tax impact snapshot (migration 0136; `flats/tax/`, `scripts/flats_tax_snapshot.py`): one jurisdiction, one tax year, the run whose green lots it priced, and every input in `params` (rate file + sources, CPR, pod unit band, new-house value and how it was derived, the RLIS TAXCODE release, the AV vintage check, counts). A one-off: nothing recomputes it | `id` (bigint); `run_id` → `runs` (set null) |
| **FlatsTaxImpactLot** | `flats.tax_impact_lots` | One green lot of a tax snapshot under Oregon Measures 5 and 50, four ways: `a_` today, `b_` one new house, `c_low_` / `c_mid_` / `c_high_` the pod partitioned into four fee-simple lots at the low end, the middle (migration 0137: the assessor's median value on the jurisdiction's new small-lot houses) and the high end of the unit value band. Per scenario: `av`, `total` (headline = permanent + urban renewal + bonds after compression; local options excluded), `local_option`, `city` (the city's permanent + bond), `city_local_option`, `ten_year` (illustrative). `green_source` says which screen called it green; `notes` holds skips, flags and compression | PK `(tax_snapshot_id, lot_id)`; cascade from both; indexed on `lot_id` |

**The last four are inboxes, not sources of truth**, and the distinction is the whole
...
`scripts/flats_drain_word_rulings.py`, `scripts/flats_drain_signatures.py`) writes
