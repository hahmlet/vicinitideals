...
| **FlatsTaxImpactLot** | `flats.tax_impact_lots` | One green lot of a tax snapshot under Oregon Measures 5 and 50, four ways: `a_` today, `b_` one new house, `c_low_` / `c_high_` the pod partitioned into four fee-simple lots at each end of the unit value band. Per scenario: `av`, `total` (headline = permanent + urban renewal + bonds after compression; local options excluded), `local_option`, `city` (the city's permanent + bond), `city_local_option`, `ten_year` (illustrative). `green_source` says which screen called it green; `notes` holds skips, flags and compression | PK `(tax_snapshot_id, lot_id)`; cascade from both; indexed on `lot_id` |
