...
3. **Any zone code nobody has ruled on?** — "Zone codes nobody has ruled
   ...
   measurement) or a `zone_rulings` entry (`alias` for the map's spelling
   of a held zone, `pocket` for another jurisdiction's zoning on this
   layer's map, `unencodable` for a district read that asks a measurement
   the model cannot take, `to_read` when the chapter is not encoded yet --
   its use table unread, or the use permitted and the dimensions unread; a
   permitted zone becomes a full zone block only when its dimensions are
