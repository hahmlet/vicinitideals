...
class OperationalInputs(Base):
    ...
    # Affordable housing — enables AMI rent tier columns in the Property module
    affordable_housing_project: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False, server_default=false()
    )
...
USE_CATEGORY_PRESETS: dict[str, tuple[str, ...]] = {
    "acquisition": (
        "All Acquisition Costs",  # catch-all for quick proformas
        "Acquisition Fee",
        "Appraisal",
        "Closing Costs",
    ...
    "soft": (
        "All Soft Costs",  # catch-all for quick proformas
